...
dxGeom::dxGeom (dSpaceID _space, int is_placeable, dxBody* newBody)
{
  initColliders();
...
#define CHECK_THIS_NOT_LOCKED(space) \
  dUASSERT (!(space->lock_count), \
	    "invalid operation for geom in locked space");
...
dxUserGeom::dxUserGeom ( ) : dxGeom (NULL, 0, NULL)
{
...
{
  dAASSERT(class_num >= dFirstUserClass && class_num <= dLastUserClass);
  type = class_num;
  int size = user_classes[type - dFirstUserClass].bytes;
  dAASSERT((signed)sizeof( this->user_data ) >= size);
...
void dInitUserGeom(dxGeom *geom, int classnum, dxSpace *space, dxBody *body)
{
  dIASSERT(geom);
  dUASSERT(( classnum >= dFirstUserClass ) && ( classnum <= dLastUserClass ), "not a custom class");

  if ( geom ){
    new(geom) dxUserGeom(classnum, space, body);
  }
}
...
dxGeom* ODE_CreateGeom(int classnum, dxSpace* space, dxBody *body)
{
  ...
  if(place)
  {
    dxUserGeom * geom = new(place) dxUserGeom(classnum, space, body);
    return geom;
  }
  return NULL;
}
...
void dGeomFree(dxGeom *g)
{
  ...
  {
    case dBoxClass:
    case dFirstUserClass:
    case dSimpleSpaceClass|dPlaneClass:
    case dHashSpaceClass|dPlaneClass:
    case dLastSpaceClass|dPlaneClass:
      if ( g->type < dFirstUserClass || user_classes[g->type - dFirstUserClass].isPlaceable )
      {
        Sys_EnterCriticalSection(CRITSECT_PHYSICS);
        Pool_Free(g, &odeGlob.geomPool);
        Sys_LeaveCriticalSection(CRITSECT_PHYSICS);
      }
      break;
    case dGeomTransformClass:
      dIASSERT(dynamic_cast<dxGeomTransform*>(g));
      dynamic_cast<dxGeomTransform*>(g)->Destruct();
      Sys_EnterCriticalSection(CRITSECT_PHYSICS);
      Pool_Free(g, &odeGlob.geomPool);
      Sys_LeaveCriticalSection(CRITSECT_PHYSICS);
      break;
    case dTriMeshClass:
      return;
    case dFirstSpaceClass:
      dIASSERT(dynamic_cast<dxSpace*>(g));
      dynamic_cast<dxSpace*>(g)->clear();
      break;
    default:
      dUASSERT(0, "other geoms are not yet supported")
      break;
  }
}

dxUserGeom* Phys_GetWorldGeom()
{
  return &odeGlob.worldGeom;
}

void ODE_GeomGetAAContainedBox(dxGeom *geom, float *mins, float *maxs)
{
  double minlength;
  double v;
  dVector3 l;
  ...
  {
    case dBoxClass:
      dGeomBoxGetLengths(geom, l);
      minlength = fminf(l[2], fminf(l[1], l[0]));
      
      dAASSERT(minlength > 0);

      v = -(minlength * 0.5);
      mins[0] = v;
      mins[1] = v;
      mins[2] = v;
      maxs[0] = minlength * 0.5;
      maxs[1] = minlength * 0.5;
      maxs[2] = minlength * 0.5;
      break;
    case dGeomTransformClass:
      ODE_GeomTransformGetAAContainedBox(geom, mins, maxs);
      break;
    case dFirstUserClass:
    case dFirstUserClass +1:
    case dFirstUserClass +2:
    case dFirstUserClass +3:
    case dLastUserClass:
      Phys_GeomUserGetAAContainedBox(geom, mins, maxs);
      break;
    default:
      dUASSERT(0, "Invalid geom");
      break;
  }
}