...
void dxProcessIslands (dxWorld *world, dReal stepsize, dstepper_fn_t stepper)
{
  ...
  body = (dxBody**) ALLOCA (world->bodyCount * sizeof(dxBody*));
  joint = (dxJoint**) ALLOCA (world->jointCount * sizeof(dxJoint*));
  ...
  int stackalloc = (world->jointCount < world->bodyCount) ? world->jointCount : world->bodyCount;
  ...
  for (bb=world->firstbody; bb; bb=(dxBody*)bb->next) {
    ...
    while (stacksize > 0) {
      ...
      dIASSERT(stacksize <= world->bodyCount);
      dIASSERT(stacksize <= world->jointCount);
    }
