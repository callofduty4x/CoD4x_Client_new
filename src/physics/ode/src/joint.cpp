			    ...
			    dVector3 anchor1, dVector3 anchor2)
{
  ...
  if (joint->node[1].body) {
    for (int j=0; j<3; j++) {
      info->c[j] = k * (a2[j] + joint->node[1].body->info.pos[j] - a1[j] - joint->node[0].body->info.pos[j]);
    }
  }
  else {
    for (int j=0; j<3; j++) {
      info->c[j] = k * (anchor2[j] - a1[j] - joint->node[0].body->info.pos[j]);
    }
  }
}
  ...
  if (powered || limit) {
    ...
    if (powered) {
      ...
      if (! limit) {
        ...
        dReal fm = fmax;
        if (vel > 0 || (vel == 0.0 && limit == 2)) fm = -fm;
				     ...
				     dReal x, dReal y, dReal z)
{
  ...
  dUASSERT(joint->typenum == J_BALL,"joint is not a ball");
...
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->typenum == J_HINGE,"joint is not a hinge");
  setAnchors (joint,x,y,z,joint->anchor1,joint->anchor2);
...
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->typenum == J_SLIDER,"joint is not a slider");
...
static void contactGetInfo1 (dxJointContact *j, dxJoint::Info1 *info)
{
  ...
  info->nub = nub;
  if ( j->debug ){
    //Com_Printf(20, "0x%p info1: m %d  nub %d\n", j, m, nub);
  }
}

void ContactGetInfo2_DebugPrint(const dxJointContact *joint, dxJoint::Info2 *info)
{
  /*
  int skip;
  int i;

  Com_Printf(20, "0x%p 2: m %d skip %d  fps %f  erp %f\n", joint, joint->the_m, info->rowskip, info->fps, info->erp);
  skip = 0;
  for ( i = 0; i < joint->the_m; ++i )
  {
    Com_Printf(20, "  %d: J = %f %f %f %f %f %f  ", i, info->J1l[skip], info->J1l[skip + 1], info->J1l[skip + 2], info->J1a[skip], info->J1a[skip + 1], info->J1a[skip + 2]);
    Com_Printf(20, "c = %f  cfm = %f  lo = %g  hi = %g  findex = %d\n", info->c[i], info->cfm[i], info->lo[i], info->hi[i], info->findex[i]);
    skip += info->rowskip;
  }
  */
}

static void contactGetInfo2 (const dxJointContact *j, const dxWorldStepInfo *stepInfo, dxJoint::Info2 *info)
{
  int i,s = info->rowskip;
  int s2 = 2*s;
  ...
  if (j->node[1].body) {
    for (i=0; i<3; i++) c2[i] = j->contact.geom.pos[i] - j->node[1].body->info.pos[i];
    info->J2l[0] = -normal[0];
    info->J2l[1] = -normal[1];
    info->J2l[2] = -normal[2];
  ...
  if (j->contact.surface.mode & dContactBounce) {
    ...
    dReal outgoing = dDOT(info->J1l,j->node[0].body->info.lvel) + dDOT(info->J1a,j->node[0].body->info.avel);
    if (j->node[1].body) {
      outgoing += dDOT(info->J2l,j->node[1].body->info.lvel) + dDOT(info->J2a,j->node[1].body->info.avel);
    }
  ...
  if (j->debug)
  {
    ContactGetInfo2_DebugPrint(j, info);
  }
...
static void hinge2GetInfo2 (dxJointHinge2 *joint, dxJoint::Info2 *info)
{
  ...
  p[0] = ax2[0] - ax1[0] * cos_angle;
  p[1] = ax2[1] - ax1[1] * cos_angle;
  p[2] = ax2[2] - ax1[2] * cos_angle;
  dCROSS (q,=,ax1,p);
...
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->typenum == J_HINGE2,"joint is not a hinge2");
  setAnchors (joint,x,y,z,joint->anchor1,joint->anchor2);
...
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->typenum == J_UNIVERSAL,"joint is not a universal");
  setAnchors (joint,x,y,z,joint->anchor1,joint->anchor2);
...
static void amotorComputeEulerAngles (dxJointAMotor *joint, dVector3 ax[3], float *angle)
{
  // assumptions:
...
static void amotorGetInfo1 (dxJointAMotor *j, dxJoint::Info1 *info)
{
  ...
  if (j->mode == dAMotorEuler) {
    ...
    amotorComputeEulerAngles (j,ax, angle);
  }else{
    angle[0] = 0;
    angle[1] = 0;
    angle[2] = 0;
  ...
  for (int i=0; i < j->num; i++) {
    if (j->limot[i].testRotationalLimit (angle[i]) || j->limot[i].fmax > 0) {
      info->m++;
    }
  }
}
...
extern "C" void dJointSetAMotorNumAxes (dxJointAMotor *joint, int num)
{
  ...
  dUASSERT(joint->typenum == J_AMOTOR,"joint is not an amotor");
  if (joint->mode == dAMotorEuler) {
    joint->num = 3;
  }
  else {
...
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->typenum == J_FIXED,"joint is not fixed");
  int i;
...
void jointInit(dxJoint *j)
{
  switch ( j->typenum )
  {
    default:
      dUASSERT(0,"bad switch type");
    case J_UNINITIALIZED:
    case J_AMOTOR:
      amotorInit((dxJointAMotor *)j);
      break;
    case J_BALL:
      ballInit ((dxJointBall *)j);
      break;
    case J_HINGE:
      hingeInit((dxJointHinge *)j);
      break;
    case J_SLIDER:
      sliderInit ((dxJointSlider *)j);
      break;
    case J_CONTACT:
      contactInit((dxJointContact *)j);
      break;
    case J_HINGE2:
      hinge2Init ((dxJointHinge2 *)j);
      break;
    case J_FIXED:
      fixedInit ((dxJointFixed *)j);
      break;
    /*
    case J_UNIVERSAL:
      break;
...
void jointGetInfo1(const dxJoint *j, dxJoint::Info1 *info)
{
  switch ( j->typenum )
  {
    default:
    ...
    case J_UNINITIALIZED:
    case J_AMOTOR:
      amotorGetInfo1((dxJointAMotor *)j, info);
      break;
    case J_BALL:
      ballGetInfo1 ((dxJointBall *)j, info);
      break;
    case J_HINGE:
      hingeGetInfo1((dxJointHinge *)j, info);
      break;
    case J_SLIDER:
      sliderGetInfo1 ((dxJointSlider *)j, info);
      break;
    case J_CONTACT:
      contactGetInfo1((dxJointContact *)j, info);
      break;
    case J_HINGE2:
      hinge2GetInfo1 ((dxJointHinge2 *)j, info);
      break;
    case J_FIXED:
      fixedGetInfo1 ((dxJointFixed *)j, info);
      break;
    /*
    case J_UNIVERSAL:
      universalGetInfo1((dxJointUniversal *)j, info);
      break;
    */
  }
}

void jointGetInfo2(dxJoint *j, dxWorldStepInfo *stepInfo, dxJoint::Info2 *info)
{
  switch ( j->typenum )
  {
    default:
    ...
    case J_UNINITIALIZED:
    case J_AMOTOR:
      amotorGetInfo2((dxJointAMotor *)j, info);
      break;
    case J_BALL:
      ballGetInfo2 ((dxJointBall *)j, info);
      break;
    case J_HINGE:
      hingeGetInfo2((dxJointHinge *)j, info);
      break;
    case J_SLIDER:
      sliderGetInfo2 ((dxJointSlider *)j, info);
      break;
    case J_CONTACT:
      contactGetInfo2((dxJointContact *)j, stepInfo, info);
      break;
    case J_HINGE2:
      hinge2GetInfo2 ((dxJointHinge2 *)j, info);
      break;
    case J_FIXED:
      fixedGetInfo2 ((dxJointFixed *)j, info);
      break;
    /*
    case J_UNIVERSAL:
      universalGetInfo1((dxJointUniversal *)j, info);
