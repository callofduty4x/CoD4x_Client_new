...
#else
VariableValue Scr_EvalVariableExtern(unsigned int id)
{
  return Scr_EvalVariable(id);
}
#endif
...
//Fix for weird GNU-GCC ABI
void Scr_EvalVariableFieldExtern(unsigned int id, VariableValue* varptr)
{
  VariableValue var = Scr_EvalVariableField(id);
  *varptr = var;
}
...
//Fix for weird GNU-GCC ABI
void Scr_FindVariableFieldExtern(unsigned int parentId, unsigned int name, VariableValue* varptr)
{
  VariableValue var = Scr_FindVariableField(parentId, name);
  *varptr = var;
}
...
{
  VariableValueInternal_u *threadvar;
  VariableValueInternal *parentValue;
  ...
  if ( id )
  {
    for ( selfNameId = FindObject(id); ; RemoveObjectVariable(selfNameId, name - SL_MAX_STRING_INDEX) )
    {
      ...
      VM_CancelNotify(threadvar->u.pointerValue, name - SL_MAX_STRING_INDEX);
...
static int Scr_MakeValuePrimitive(unsigned int parentId)
{
  VariableValueInternal *parentValue;
  unsigned int name;
  VariableValueInternal *entryValue;
...
uint32_t Scr_GetEntityIdRefExtern(unsigned int entId)
{
  scr_entref_t ref;
...
static void Scr_ResetScrVarDebugPub( )
{
#ifndef NDEBUG
...
static void Scr_InitVariableRange(unsigned int begin, unsigned int end)
{
  unsigned int index;
...
static void Scr_CheckLeakRange(unsigned int begin, unsigned int end)
{
  VariableValueInternal *entry;
  int index;
  VariableValueInternal *value;
...
void Scr_AddFields_LoadObj(const char *path, const char *extension)
{
  ...
  files = FS_ListFiles(path, extension, FS_LIST_PURE_ONLY, &numFiles, TRACK_MISC);
  gScrVarPub.fieldBuffer = (char*)TempMalloc(1);
  *(char*)gScrVarPub.fieldBuffer = 0;
...
static unsigned int Scr_FindArrayIndex(unsigned int parentId, VariableValue *index)
{
  unsigned int id;
