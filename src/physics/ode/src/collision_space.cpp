...
#define GEOM_ENABLED(g) ((g)->gflags & 8)

//****************************************************************************
// dxSpace

dxSpace::dxSpace (dSpaceID _space) : dxGeom (_space, 0, 0)
{
  count = 0;
  first = 0;
  cleanup = 1;
...
dxSimpleSpace::dxSimpleSpace ( ) : dxSpace (NULL)
{
  type = dSimpleSpaceClass;
}
...
dxSimpleSpace* dGetSimpleSpace(PhysWorld worldIndex)
{
  return new(&odeGlob.space[worldIndex]) dxSimpleSpace(0);
}

#define GEOM_BODY_DISABLED( x ) (x->body && (x->body->flags & 4))

void ODE_CollideSimpleSpaceWithGeomNoAABBTest(dxSpace *space, dxGeom *geom, void *data)
{
  dxGeom *geom2;
  ...
  dAASSERT(!GEOM_BODY_DISABLED( geom ));
  dAASSERT(GEOM_ENABLED( geom ));
