...
extern clipMap_t cm;
void CM_LoadMapFromBsp(const char *name, char a2);
...
#ifndef BSPC
byte *Com_GetBspLump(enum LumpType type, unsigned int elemSize, unsigned int *count);
bool Com_BspHasLump(enum LumpType type);
#endif

void *CM_Hunk_Alloc(int numBytes, const char *what);
unsigned int Com_GetBspVersion();
void CM_Hunk_ClearTempMemoryHigh();
void CM_Hunk_CheckTempMemoryHighClear();
char *CM_Hunk_AllocateTempMemoryHigh(int size);
