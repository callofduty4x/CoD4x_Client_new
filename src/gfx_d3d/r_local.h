...
extern dvar_s* r_drawSun;
extern dvar_s* r_loadForRenderer;
extern dvar_s* r_rendererInUse;
extern dvar_s* r_logFile;
extern dvar_s* r_colorMap;
extern dvar_s* r_normalMap;
extern dvar_s* r_specularMap;
extern dvar_s* r_preloadShaders;
extern dvar_s* r_picmip_water;
extern dvar_s* r_vsync;
extern dvar_s* r_mode;
extern dvar_s* r_displayRefresh;
extern dvar_s* r_aaSamples;
extern dvar_s* sm_enable;
extern dvar_s* r_customMode;
extern dvar_s* r_multiGpu;
extern dvar_s* r_gpuSync;
extern dvar_s* r_aspectRatio;
extern dvar_s* r_floatz;
extern dvar_s* r_gamma;
extern dvar_s* r_ignoreHwGamma;
extern dvar_s* r_polygonOffsetBias;
extern dvar_s* r_polygonOffsetScale;
extern dvar_s* sm_polygonOffsetBias;
extern dvar_s* sm_polygonOffsetScale;
extern dvar_s* r_aaAlpha;
extern dvar_s* r_detail;
extern dvar_s* r_specular;
extern dvar_s* r_normal;
extern dvar_s* r_zFeather;
extern dvar_s* r_outdoor;
extern dvar_s* r_envMapOverride;
extern dvar_s* r_drawWater;
extern dvar_s* r_lightMap;
extern dvar_s* sc_enable;
extern dvar_s* r_scaleViewport;
extern dvar_s* r_depthPrepass;
...
void R_ChangeState_0(GfxCmdBufState*, unsigned int);
void R_ChangeState_1(GfxCmdBufState*, unsigned int);
...
void R_SetSamplerState(GfxCmdBufState *state, unsigned int samplerIndex, unsigned char samplerState);
void R_UnbindImage(GfxCmdBufState *state, GfxImage *image);
void R_SetDrawSurfsShadowableLight(GfxCmdBufSourceState *source, const GfxDrawSurfListInfo *info);
...
void R_SetShadowableLight(GfxCmdBufSourceState*, unsigned int, GfxViewInfo const*);
void R_InitContext(const GfxBackEndData *data, GfxCmdBuf *cmdBuf);
void R_SetQuadMeshData(GfxMeshData *mesh, float x, float y, float w, float h, float s0, float t0, float s1, float t1, unsigned long color);
...
#define D3DCALLNOLOCKNOERROR(EXP) \
  ({HRESULT hr;\
  do { \
    _RB_LogPrintf(#EXP"\n"); \
    hr = EXP; \
...
#define D3DCALLNOLOCKNOERRORNORETVAL(EXP) \
  do { \
    _RB_LogPrintf(#EXP"\n"); \
    EXP; \
  }while(0)\

#define D3DCALLNOERROR(EXP) \
do { \
  int semaphore;\
  semaphore = R_AcquireDXDeviceOwnership(0);\
  EXP; \
...
#define D3DCALLNAKED(EXP) \
        ({int retval; retval = EXP; retval;})

#define D3DRELEASEBUFFER(EXP) \
  do { \
    _RB_LogPrintf(#EXP "->Release()\n"); \
    auto *varCopy = EXP; \
    EXP = 0;\
    R_ReleaseAndSetNULL(varCopy, #EXP, __FILE__, __LINE__);\
  }while(0)\
