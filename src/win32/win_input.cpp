...
// perform button actions
	for  ( button = 0 ; button < 5 ; button++ )
	{
		...
		if(mstate & ( 1 << button )){
			Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MOUSE1 + button, qtrue, 0, NULL );
		}else{
			Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MOUSE1 + button, qfalse, 0, NULL );
		}
	}
...
void IN_StartupMouse()
{
	...
	{
		Dvar_SetBool(raw_input, qfalse);
		Dvar_ClearModified(raw_input);
	}
	if ( in_mouse->current.enabled )
	{
...
void IN_Init( void ) {
	...
	raw_input = Dvar_RegisterBool("raw_input", qfalse, CVAR_LATCH | CVAR_ARCHIVE, "Use raw input for input devices");
	in_mouse = Dvar_RegisterInt("in_mouse", 1, 0, 1, CVAR_LATCH | CVAR_ARCHIVE, "Initialize the mouse");
	IN_Startup();
...
void IN_RawEvent(LPARAM lParam)
{
	...
	if ( raw_input->current.enabled && IN_IsForegroundWindow() )
	{
...
{
	return Dvar_HasLatchedValue(raw_input);
}
