	...
	if (psLine)
	{
		...
		if (!Q_stricmpn(sSE_KEYWORD_LANG, psLine, strlen(sSE_KEYWORD_LANG)))
		{
			...
			if ( psReference[0] )
			{
				...
				if ( !IsStringFormatCorrect(psSentence) )
				{
					psErrorMessage = va("Illegal string format \"%s\"", psSentence);
				}
...
LPCSTR Leetify( LPCSTR psString )
{
	static string str;
	str = psString;
	if (sp_leet->current.integer == 42)	// very specific test, so you won't hit it accidentally
	{
		static const
...
LPCSTR SE_GetString_LoadObj( LPCSTR psPackageAndStringReference )
{
	...
	if (itEntry != TheStringPackage->m_StringEntries.end())
	{
		...
		if ( se_debug->current.integer && TheStringPackage->m_bLoadDebug )
		{
