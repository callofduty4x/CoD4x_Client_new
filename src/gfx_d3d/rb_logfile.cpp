...
{
  assert(r_logFile->current.integer);
  if ( RB_IsLogging() )
  {
...
void RB_LogBlend(const char *format, unsigned int blend)
{
...
void RB_UpdateLogging()
{
  if ( RB_IsLogging() )
  {
    Dvar_SetInt(r_logFile, r_logFile->current.integer - 1);
  }
  if ( r_logFile->current.integer )
  {
    RB_OpenLogFile();
